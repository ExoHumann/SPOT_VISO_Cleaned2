...
class BaseObject:
    ...
    def get_input_for_visualisation(
        self,
        axis_data: List[Dict],
        cross_section_objects: List["CrossSection"],
        json_file_path: str = None,
        axis_rotation: float = None,
        colors: Dict[str, str] = None
    ) -> Dict:
        return create_input_for_visualisation(self, axis_data, cross_section_objects, json_file_path, axis_rotation, colors)
...
class CrossSection:
    ...
    @property
    def points_by_name(self) -> dict:
        # Builds once per instance; subsequent calls are O(1)
...
def create_input_for_visualisation(
    ...
    # Combine object's station_value with user_stations
    if obj.class_name == 'DeckObject':
        station_value = getattr(obj, 'station_value', [])
    else:
        station_value = getattr(obj, 'internal_station_value', [])
    if isinstance(station_value, (int, float)):
        station_value = [float(station_value)]
    user_stations = getattr(obj, 'user_stations', [])
    base_breaks = sorted(set(map(float, station_value + user_stations)))
    ...
    # Find axis dict
    axis_dict = None
    axis_name = getattr(obj, 'axis_name', '')
    for data in axis_data:
        if (data.get(class_key) == 'Axis' and data.get(name_key, '').lower() == axis_name.lower()):
            axis_dict = data
            break
